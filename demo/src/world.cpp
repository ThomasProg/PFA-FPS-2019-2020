...
void World::makeNewLevel()
{
    ...
    for (uint i = 0; i < 10; i++)
    {
        entityGroup.addTree({{6.f+std::sin(float(i)*5)*2.f, -30, 0+float(i)*5}, {0.f,0,0}, {1,1,1}});
        entityGroup.addTree({{-6.f-std::sin(10+float(i)*5)*2.f, -30, 0+float(i)*5}, {0.f,0,0}, {1,1,1}});
    }
