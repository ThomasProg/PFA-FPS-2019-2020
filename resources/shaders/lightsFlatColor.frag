...
void main()
{
    vec4 intensity = vec4(0.0,0.0,0.0,1);

    for (int i = 0; i < nbCurrentLights; i++)
    {
      switch(lights[i].lightType)
      {
        case 0 : 
          intensity += getPointLightIntensity(i); 
          break;
        case 1 : 
          intensity += getDirectionalIntensity(i); 
          break;
        case 2 : 
          intensity += getFlashLightIntensity(i); 
          break;
        case 3 : 
          intensity += getSpotLightIntensity(i); 
          break;
      }
    }

    out_color = color * intensity.bgra;
}
